...
export default class PersonalInformationForm extends Component{
	...
	validationForm(){
		...
		let i_name, i_documentType, i_documentNum, i_position, i_email, i_phone, i_location, 
			i_civilStatus, i_goodsSeparationY, i_goodsSeparationN, i_registryNum, i_registryOffice, 
		...
		this.props.enterpriseInProcessData.partners.map((partner, index)=>{
			...
			if(errorFlag == 0){
				...
				if(partner.user.civilStatus == "Casado"){
					...
					i_goodsSeparationY = ReactDom.findDOMNode(this.refs.inputRadioButtonY);
					i_goodsSeparationN = ReactDom.findDOMNode(this.refs.inputRadioButtonN);
				...
				if(errorFlag == 1){
					this.switchPartnerSelected(errorPartnerPos);
				}
			}
			if(index == this.props.enterpriseInProcessData.partners.length -1){
...
*/
	...
	render(){
		...
		if(this.state.redirect){
			return <Redirect to="/fecha-firma"/>
		}
		else{
			...
			let coupleBlockA, coupleBlockB;

			let goodsInstruction;
			...
			if(this.props.enterpriseInProcessData.isItGoodsCapital == true){
				...
				goodsInstruction = <label className="smallContent goodsInstruction">En caso de equipos electrónicos ingresar marca, modelo y número de serie.</label>
			}
			...
			if(dataReaderPartner.user){
				...
				if(dataReaderPartner.user.civilStatus == "Casado"){
					...
					inputGoodsSeparation = <div className="wrapperInputRadio">
												<label>
													<input type="radio" name="goodsSeparation" onChange={this.props.selectHandler.bind()} checked={dataReaderPartner.user.goodsSeparation === "1"} value="1" data-pos={this.state.partnerSelected} ref="inputRadioButtonY" className="inputRadioButtonPersonalInfo"/>Si
												</label>
												<label>
													<input type="radio" name="goodsSeparation" onChange={this.props.selectHandler.bind()} checked={dataReaderPartner.user.goodsSeparation === "0"} value="0" data-pos={this.state.partnerSelected} ref="inputRadioButtonN" className="inputRadioButtonPersonalInfo"/>No
												</label>
					...
					coupleBlockA = <div className="gridFormShort">
					...
					coupleBlockB = <div className="coupleBlockB">
					...
					coupleBlockA = <div className="gridFormShort"></div>
				}
								...
								<div className="gridForm">
									...
									<div className="wrapperBtnForm">
										<div className="gridFormLarge wrapperBtnNext">
											<a><div className="btnNext" onClick={this.handlePersonalInformationFormSubmit.bind(this)}>Siguiente</div></a>
										</div>
